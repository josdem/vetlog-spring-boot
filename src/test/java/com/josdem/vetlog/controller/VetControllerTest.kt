...
class VetControllerTest {
    ...
    fun shouldShowCreateVetForm(testInfo : TestInfo) {
    ...
    fun shouldSearchPetsByUser(testInfo : TestInfo) {
        log.info("Running: {}", testInfo.displayName)
        registerPet()
        val request = post("/vet/search")
            .with(csrf())
    ...
    @WithMockUser(username = "admin", password = "12345678", roles = ["ADMIN"])
    fun shouldSearchPetsByMobile(testInfo : TestInfo) {
        log.info("Running: {}", testInfo.displayName)
        registerPet()
        val request = post("/vet/search")
            .with(csrf())
